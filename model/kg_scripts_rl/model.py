...
class transformers_model(nn.Module):
    ...
    def sample(self, enc_inputs, mask_encoder_input, dec_inputs, mask_decoder_input, hidden=None, random_sample=False):
        """
        sampling for RL training
        """
        # outputs, dec_init_state = self.encode(enc_inputs, hidden)
        print(enc_inputs.size())
        print(mask_encoder_input.size())
        encoder_hidden_states = self.encoder(enc_inputs, mask_encoder_input)

        encoder_hidden_states = encoder_hidden_states.last_hidden_state

        batch_size, max_len = dec_inputs.size()
        print('batch_size',batch_size)
        # pred_log_logits = torch.zeros((batch_size, max_len, self.vocab_size))  # zeros equal to padding idx
        ...
        out = self.decoder(dec_inputs, mask_decoder_input, encoder_hidden_states=encoder_hidden_states, output_attentions=True)
        log_logits = out.last_hidden_state
        print('log_logits',log_logits.size())

        if random_sample:
            dec_inputs = torch.multinomial(torch.exp(log_logits.view(-1, log_logits.size(2))), num_samples=1).view(batch_size, max_len, -1)
        else:
            dec_inputs = torch.argmax(log_logits, dim=-1)#.view(-1)

        print('dec_inputs',dec_inputs.size())

        # pred_word[:, i] = dec_inputs

        return log_logits, dec_inputs
