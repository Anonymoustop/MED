...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_rl', default=0, type=int, required=False)
    ...
    parser.add_argument('--vocab_path', default='../../preprocessing_scripts/kg/biobert/vocab/new_med_vocab.txt', type=str, required=False)
    ...
    def collect_rl_metrics(sample_outputs, sample_logits, greedy_outputs, greedy_logits, target):
        """
        ...
        greedy_sample = greedy_outputs

        # cal reward
        sample_reward, _ = reward_fn(sample, target)
        greedy_reward, bleu_score = reward_fn(greedy_sample, target)
        reward = sample_reward - greedy_reward

        # cal RL loss
        # sample_log_prob = nll_loss(logits, sample, mask=sample.ne(self.padding_idx),
                                        # reduction=False, matrix=False)  # [batch_size, max_len]
        sample_log_prob = util.sequence_cross_entropy_with_logits(logits, sample, sample.ne(0), average="token")
    ...
    for epoch in range(epochs):
        ...
        for batch in train_dataloader:
            ...
            print('rl',args.use_rl)
            if args.use_rl:
                target = decoder_input[:, 1:].contiguous() 
                print(target.size())
                sample_logits, sample_outputs = model.sample(encoder_input, mask_encoder_input, decoder_input, mask_decoder_input, random_sample=True)
                with torch.no_grad():
                    greedy_logits, greedy_outputs = model.sample(encoder_input, mask_encoder_input, decoder_input, mask_decoder_input, random_sample=False)
                loss = collect_rl_metrics(sample_outputs, sample_logits, greedy_outputs, greedy_logits, target)
